...
export function Footer() {
  return (
    <footer className="vox-footer relative z-10">
      ...
      <div className="mx-auto max-w-7xl px-6 py-12">
        <div className="grid gap-10 lg:grid-cols-[1.2fr_1.8fr] lg:gap-16">
          <div className="flex flex-col items-center text-center lg:items-start lg:text-left">
            ...
            <p className="mt-5 max-w-sm text-sm font-light leading-6 text-muted">
              Vendor risk evidence for SOC 2 and ISO 27001 audits, with the broader TPRM workflows teams need after the audit.
            </p>
            <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-accent/25 bg-accent/10 px-3 py-1.5 text-xs text-accent shadow-card">
              <span className="h-1.5 w-1.5 rounded-full bg-accent/100" />
          ...
          <div className="grid gap-8 text-center sm:grid-cols-3 sm:text-left">
            {Object.entries(footerLinks).map(([heading, links]) => (
              <div key={heading}>
                <p className="mb-4 font-mono text-[10px] font-medium uppercase tracking-[0.12em] text-muted">
                  {heading}
                </p>
                <div className="flex flex-col gap-3 text-sm font-light text-muted">
                  {links.map((link) => (
                    <SmartLink key={link.href} href={link.href} className="transition-colors hover:text-accent">
                      {link.label}
                    </SmartLink>
                  ))}
                </div>
